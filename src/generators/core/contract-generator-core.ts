 ...
 * When called from Nx wrapper, receives pre-computed paths from standardizeGeneratorOptions().
 * When called directly (e.g., from CLI), uses workspace detection to compute paths.
 */
export interface ContractGeneratorCoreOptions {
  ...
  // Pre-computed paths (when called from Nx wrapper)
...
export function generateContractCore(
  ...
  return Effect.gen(function*() {
    ...
    // 2. Determine paths (use pre-computed from Nx wrapper if available, else calculate)
    let projectName: string
    let projectRoot: string
    let packageName: string
    let sourceRoot: string
    let offsetFromRoot: string
    
    if (options.projectName && options.projectRoot && options.packageName && options.sourceRoot) {
      // Pre-computed paths from Nx wrapper (infrastructure already generated)
      projectName = options.projectName
      projectRoot = options.projectRoot
      packageName = options.packageName
      sourceRoot = options.sourceRoot
      offsetFromRoot = calculateOffsetFromRoot(projectRoot)
    } else {
      // Calculate paths (for direct/CLI calls)
      const nameVariants = createNamingVariants(options.name)
      projectName = `contract-${nameVariants.fileName}`
      packageName = `${workspaceConfig.scope}/${projectName}`
    ...
    // 3. Generate naming variants (always needed for templates)
    const nameVariants = createNamingVariants(options.name)
    ...
    // 5. Prepare entities list (needed for templates)
    ...
    // NOTE: Infrastructure files (package.json, tsconfig, etc.) are generated by the Nx wrapper
    // via generateLibraryFiles() BEFORE this function is called. This function only generates
    // domain-specific files (entities, errors, ports, etc.).
